...
- **The founder profile.** All four stages built — [07](./07-founder-profile.md). The load-bearing-inputs
  ...
  and expertise-as-evidence in the challenge loop (one tier, in domain only, never past D-5). Built:
...
- **The rest of the web UI.** The shell, the interview, the challenge flow and the turn loop are in.
  ...
  Every income-statement figure now opens onto its own arithmetic (§16 Q3, stage 1 of
  [08-show-the-math.md](./08-show-the-math.md)): the demand chain that made revenue, each cost line's
  formula, the subtotals. The derivations are recorded by the code that computes them — `ctx.derive`
  beside `ctx.p` — and never rebuilt in the view, which would be a second implementation of every
  formula kept in sync by hope. Off in the property suite, so the 240,000-quarter run pays nothing.
  Stage 2 is the tiles, the balance sheet and the cash flow.
  Still CLI-or-nothing: the crisis policy editor; the CLI's own turn loop does not
  yet use `selectAxis`, and its advisor does not yet surface `orderedCommands`. The math panels are
  web-only — the CLI has the data and no affordance.
